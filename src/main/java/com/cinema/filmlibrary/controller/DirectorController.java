...
public class DirectorController {
     ...
     */
    @Operation(summary = "Add director to film", description = "Creates and returns the director added to the film",
            responses = {
    ...
    public Director addDirector(
            @Parameter(description = "Director object to add to the film") @RequestBody Director director,
     ...
     */
    @Operation(summary = "Update director", description = "Updates the information of an existing director",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Director updated"),
                    @ApiResponse(responseCode = "404", description = "Director not found",
                            content = @Content(schema = @Schema(example = "{ \"error\": \"Director not found\" }"))),
                    @ApiResponse(responseCode = "500", description = "Internal server error",
                            content = @Content(schema = @Schema(example = "{ \"error\": \"Internal server error\" }")))
            })
    @PutMapping("/{directorId}")
    public Director updateDirector(
            @Parameter(description = "ID of the director to update", example = "1") @PathVariable Long directorId,
            @Parameter(description = "Updated director object") @RequestBody Director director) {
     ...
     */
    @Operation(summary = "Remove director from film", description = "Removes a director from a film",
            responses = {
                    ...
                    @ApiResponse(responseCode = "404", description = "Director or Film not found",
                            content = @Content(schema = @Schema(example = "{ \"error\": \"Director or Film not found\" }"))),
                    @ApiResponse(responseCode = "500", description = "Internal server error",
                            content = @Content(schema = @Schema(example = "{ \"error\": \"Internal server error\" }")))
            })
    @DeleteMapping("/{directorId}")
    public void removeDirector(
            @Parameter(description = "ID of the director to remove", example = "1") @PathVariable Long directorId,
            @Parameter(description = "ID of the film") @PathVariable Long filmId) {
     ...
     */
    @Operation(summary = "Get director of a film", description = "Returns the director who worked on the specified film",
            responses = {
    ...
    public Director getFilmDirector(
            @Parameter(description = "ID of the director", example = "1") @PathVariable Long directorId,
            @Parameter(description = "ID of the film") @PathVariable Long filmId) {
     ...
     */
    @Operation(summary = "Get all directors", description = "Returns a list of all directors in the system",
            responses = {
